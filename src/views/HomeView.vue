...
<script>
  export default{
    ...
    mounted(){
      ...
      const year = { year: 'numeric' }//換算日期的條件
      // const weekday = {weekday: 'long'} //星期幾
      const mounth = {month: 'long'}
      ...
      const today = currentDate.toLocaleString(undefined, day).slice(this.length,-1).padStart(2,"0") 
      const tomounth = currentDate.toLocaleString(undefined, mounth).slice(this.length,-1).padStart(2,"0") 
      const toyear = currentDate.toLocaleString(undefined, year).slice(this.length,-1).padStart(4,"0") 
      // console.log(today,tomounth,toyear) //確認過只剩日期的數字了
      const defaultDate = [toyear,tomounth,today].join('-')
      ...
      const  finDay = sevenDatetime.toLocaleString(undefined,day).slice(this.length,-1).padStart(2,"0")
      // 最後面的padstart 是指定字串的長度，如果字串不夠就補後面的參數填充，要填入日期因為是2號前面沒補0所以填充失敗
      // 用padstart強制限定日期長度要2位數，如果前面不夠就補個0在前面變成02，測試可行!!其他欄位也跟著補上了
      const finMounth = sevenDatetime.toLocaleString(undefined, mounth).slice(this.length,-1).padStart(2,"0")
      const finYear = sevenDatetime.toLocaleString(undefined, year).slice(this.length,-1).padStart(4,"0") //西元年所以至少要4位數
      const defaultFinDate=[finYear,finMounth,finDay].join('-')
      endDate.value = defaultFinDate
    }
  }
</script>
